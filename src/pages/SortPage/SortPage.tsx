...
export const SortPage = ({ title, type }: SearchPageProps) => {
  ...
  useEffect(() => {
    ...
    fetch(
      `https://api.kinopoisk.dev/v1.3/movie?poster.previewUrl=!null${searchSort}&sortField[]=votes.kp&sortType[]=-1&sortType[]=-1&page=${page}&limit=21&type=${type}&rating.kp=${rating}&year=${year}` +
        searchGenres,
      {
        headers: {
          'X-API-KEY': import.meta.env.VITE_API_KEY,
